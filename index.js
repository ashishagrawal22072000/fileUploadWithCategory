  ...
  file: (req, file) => {
    ...
    return new Promise((resolve, reject) => {
      if (!file) {
        ...
        const fileInfo = {
          filename: req.userID + file.originalname,
          bucketName: "files",
          metadata: {
            user: req.userID,
...
const upload = multer({
  ...
  fileFilter: async function (req, file, cb) {
    ...
    gfs.files
      .find({ filename: req.userID + file.originalname })
      .toArray((err, files) => {
        console.log(files);
        if (files.length > 0) {
          req.fileValidationError =
            "you can't upload more than one file at once.";
...
app.get("/file/:id", isAuth, (req, res) => {
  gfs.files.find().toArray((err, files) => {
    if (!files || files.length == 0) {
      console.log(err);
    } else {
      console.log(files);
      const data = files.filter((ele) => {
        return ele.metadata.user == req.params.id;
      });
      console.log("files in files api", data);
      res.status(200).json({ data: data });
    }
  });
});

app.get("/user_file/:id", isAuth, (req, res) => {
  gfs.files.find().toArray((err, files) => {
    if (!files || files.length == 0) {
      console.log(err);
    } else {
